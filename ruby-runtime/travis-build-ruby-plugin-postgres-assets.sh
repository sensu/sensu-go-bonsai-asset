...
if [[ -z "$2" ]]; then 
  echo "Parameter 2, PLATFORMS is empty, using default set" ; platforms=( debian centos debian9 centos7 centos6 ); 
else
...
if [ -d dist ]; then
  ...
  docker build --build-arg "ASSET_GEM=${GEM_NAME}" --build-arg "GIT_REPO=${GIT_REPO}"  --build-arg "GIT_REF=${GIT_REF}" -t ruby-plugin-${platform} -f "${WDIR}/ruby-runtime/Dockerfile.postgres.${platform}" .
